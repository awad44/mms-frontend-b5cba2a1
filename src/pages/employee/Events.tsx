...
import { Event, EventAudience, UserRole } from '@/types';
...
export default function EmployeeEvents() {
  ...
  // Employees can see public events + role-specific events + all_employees events
  const getAccessibleEvents = () => {
    const userRole = user?.role as UserRole;
    
    return mockEvents.filter((event) => {
      // All employees can see public events
      if (event.target_audience === 'public') return true;
      // All employees can see all_employees events
      if (event.target_audience === 'all_employees') return true;
      // Role-specific events
      if (event.target_audience === userRole) return true;
      return false;
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">View public and department events</p>
        </div>
      </div>
